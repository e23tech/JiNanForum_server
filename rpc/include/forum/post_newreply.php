...
if(!submitcheck('replysubmit', 0, $seccodecheck, $secqaacheck)) {
	...
	if($censormod || $_G['forum']['modnewposts'] == 2) {
		$modnewreplies = 1;
	}
	if($_G['group']['allowdirectpost'] == 1 || $_G['group']['allowdirectpost'] == 3) {
		$modnewreplies = 0;
	}
...
/* echo $_G['group']['allowdirectpost'];
echo $pinvisible,'ok'; exit; */
	
	//»Ø¸´Â¥²ã
	if($_G['gp_pid']) {
		
		$post = DB::getRow("SELECT dateline, author, message FROM ".DB::table('forum_post')." 
								WHERE pid='".$_G['gp_pid']."'");
		
		$post['dateline'] = date('Y-m-d H:i:s', $post['dateline']);
		$post['message'] = cutstr($post['message'], 100);			
		//print_r($post); 
		
		$quote = '[quote][size=2][color=#999999]'.lang('forum/misc','post_reply_quote').' [/color][url=forum.php?mod=redirect&goto=findpost&pid='.$_G['gp_pid'].'&ptid='.$_G['tid']."][img]static/image/common/back.gif[/img][/url][/size]\r\n".$post['message'].'[/quote]'."\r\n";
		
		$quote = str_replace('{author}', $post['author'], $quote);
		$quote = str_replace('{time}', $post['dateline'], $quote);
	...
	if($pid) {
		...
		if($_GET['longitude'] && $_GET['latitude']) {
			$address = get_address_by_location($_GET['longitude'], $_GET['latitude']);
		}	
		
		DB::insert('zywx_forum_postfield', array(
			'pid' => $pid,
