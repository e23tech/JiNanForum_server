	...
	switch($idtype) {
		...
		case 'picid':
			...
			if(!ckfriend($album['uid'], $album['friend'], $album['target_ids'])) {
				$msg = lang('message', 'no_privilege_ckfriend_pic');
				if(CHARSET != 'utf-8') {
					$msg = gbktoutf8($msg);
				}
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
				...
				if($cookievalue != md5(md5($album['password']))) {
					$msg = lang('message', 'no_privilege_ckpassword_pic');
					if(CHARSET != 'utf-8') {
						$msg = gbktoutf8($msg);
					}
					jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
		...
		case 'blogid':
			...
			if(!ckfriend($blog['uid'], $blog['friend'], $blog['target_ids'])) {
				...
				if($cookievalue != md5(md5($blog['password']))) {
					$msg = lang('message', 'no_privilege_ckpassword_blog');
					if(CHARSET != 'utf-8') {
						$msg = gbktoutf8($msg);
					}
					jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			...
			if(!empty($blog['noreply'])) {
				$msg = lang('message', 'do_not_accept_comments');
				if(CHARSET != 'utf-8') {
					$msg = gbktoutf8($msg);
				}
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
		...
		case 'sid':
			...
			if(empty($share)) {
				$msg = lang('message', 'sharing_does_not_exist');
				if(CHARSET != 'utf-8') {
					$msg = gbktoutf8($msg);
				}
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			...
			break;
		default:
			$msg = lang('message', 'non_normal_operation');
			if(CHARSET != 'utf-8') {
				$msg = gbktoutf8($msg);
			}
			jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
