...
if($action == 'index') {
	...
	if($op == 'alluser') {
		$alluserlist = groupuserlist($_G['fid'], 'lastupdate', $perpage, $start, "", '', $onlinemember['list']);
		$multipage = multi($_G['forum']['membernum'], $perpage, $page, '?mod=group&action=memberlist&op=alluser&fid='.$_G['fid']);
	...
	if(!empty($membermaximum)) {
		...
		if($curnum >= $membermaximum) {
			$msg = rpclang('forum', 'group_member_maximum');
			jsonexit("{\"message\":\"$msg\"}");
		}
	}
	if($groupuser['uid']) {
		$msg = rpclang('forum', 'group_has_joined');
		jsonexit("{\"message\":\"$msg\"}");
	} else {
		$modmember = 4;
