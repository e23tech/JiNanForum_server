...
class discuz_core {
	...
	function _init_env() {
		...
		define('DISCUZ_ROOT', substr(dirname(__FILE__), 0, -28));
		...
		if(!defined('DISCUZ_CORE_FUNCTION') && !include(RPC_DIR.'/function/function_core.php')) {
		...
		$_G['siteurl'] = str_replace('source/plugin/zywx/rpc/', '', $_G['siteurl']);
	...
	function _init_config() {
		...
		define('STATICURL', !empty($_config['output']['staticurl']) ? $this->var['siteurl'].$_config['output']['staticurl'] : 'static/');
	...
	function _init_output() {
		...
		$allowgzip = 0 && empty($this->var['inajax']) && $this->var['mod'] != 'attachment' && EXT_OBGZIP;
		...
		if($this->config['output']['forceheader']) {
			...
			//if(CHARSET == 'gbk')  header('Content-Type: text/html; charset=gbk');
		}
