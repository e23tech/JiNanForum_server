...
if($_G['forum']['status'] == 3) {
	...
	if($status == -1) {
		$message = lang('message', 'forum_not_group');
		jsonexit("{\"message\":\"$message\"}");
	} elseif($status == 1) {
		$message = lang('message', 'forum_group_status_off');
		jsonexit("{\"message\":\"$message\"}");
	} elseif($status == 2) {
		$message = lang('message', 'forum_group_noallowed');
		jsonexit("{\"message\":\"$message\"}");
	} elseif($status == 3) {
		$message = lang('message', 'forum_group_moderated');
		jsonexit("{\"message\":\"$message\"}");
	} elseif($status == 4) {
		if($_G['uid']) {
			$message = rpclang('forum', 'forum_group_not_groupmember');
			jsonexit("{\"message\":\"$message\"}");
		} else {
			$message = lang('message', 'forum_group_not_groupmember_guest');
			jsonexit("{\"message\":\"$message\"}");
		}
	} elseif($status == 5) {
		$message = lang('message', 'forum_group_moderated');
...
if($_G['gp_action'] == 'edit' || $_G['gp_action'] == 'reply') {
	...
	if($thread = DB::fetch_first("SELECT * FROM ".DB::table('forum_thread')." WHERE tid='$_G[tid]'".($_G['forum_auditstatuson'] ? '' : " AND (displayorder>='0' OR (displayorder IN ('-4', '-2') AND authorid='$_G[uid]'))"))) {
		...
		if($thread['readperm'] && $thread['readperm'] > $_G['group']['readaccess'] && !$_G['forum']['ismoderator'] && $thread['authorid'] != $_G['uid']) {
			$message = lang('message', 'thread_nopermission');
			jsonexit("{\"message\":\"$message\"}");
		}
		...
		$message = lang('message', 'thread_nonexistence');
		jsonexit("{\"message\":\"$message\"}");
	}

	if($_G['gp_action'] == 'reply' && ($thread['closed'] == 1) && !$_G['forum']['ismoderator']) {
		$message = lang('message', 'post_thread_closed');
		jsonexit("{\"message\":\"$message\"}");
	}
}
...
if($_G['forum']['password'] && $_G['forum']['password'] != $_G['cookie']['fidpw'.$_G['fid']]) {
	$message = lang('message', 'forum_passwd');
	jsonexit("{\"message\":\"$message\"}");
}

if(empty($_G['forum']['allowview'])) {
	if(!$_G['forum']['viewperm'] && !$_G['group']['readaccess']) {
		$message = rpclang('forum', 'group_nopermission');
		jsonexit("{\"message\":\"$message\"}");
	} elseif($_G['forum']['viewperm'] && !forumperm($_G['forum']['viewperm'])) {
		$message = lang('message', 'viewperm');
		jsonexit("{\"message\":\"$message\"}");
	}
} elseif($_G['forum']['allowview'] == -1) {
	$message = lang('message', 'forum_access_view_disallow');
	jsonexit("{\"message\":\"$message\"}");
}
...
if(!$_G['adminid'] && $_G['setting']['newbiespan'] && (!getuserprofile('lastpost') || TIMESTAMP - getuserprofile('lastpost') < $_G['setting']['newbiespan'] * 60) && TIMESTAMP - $_G['member']['regdate'] < $_G['setting']['newbiespan'] * 60) {
	$message = lang('message', 'post_newbie_span');
	jsonexit("{\"message\":\"$message\"}");
}
...
if(empty($bbcodeoff) && !$_G['group']['allowhidecode'] && !empty($message) && preg_match("/\[hide=?\d*\].*?\[\/hide\]/is", preg_replace("/(\[code\](.+?)\[\/code\])/is", ' ', $message))) {
	$message = lang('message', 'post_hide_nopermission');
	jsonexit("{\"message\":\"$message\"}");
}

$censormod = censormod($subject."\t".$message);
$_G['group']['allowdirectpost'] = DB::getOne("SELECT allowdirectpost FROM ".DB::table('common_usergroup_field')." WHERE groupid='".$_G['group']['groupid']."'");
...
if($_G['gp_action'] == 'newthread') {
	...
	require_once RPC_DIR . '/include/forum/post_newthread.php';
} elseif($_G['gp_action'] == 'reply') {
	$navtitle .= ' - '.$thread['subject'].' - '.$_G['forum']['name'];
	require_once RPC_DIR . '/include/forum/post_newreply.php';
