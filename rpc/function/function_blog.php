...
//		$picids = array_keys($POST['picids']);
	...
	if($olds) {
		...
		if($_G['gp_longitude'] && $_G['gp_latitude'] && $blogid) {
			$address = get_address_by_location($_G['gp_longitude'], $_G['gp_latitude']);
			
			DB::insert('zywx_home_blogfield', array(
				'blogid' => $blogid,
				'longitude' => $_G['gp_longitude'],
				'latitude' => $_G['gp_latitude'],
				'device' => $_G['gp_device'],
				'address' => $address,
				'dateline' => $_G['timestamp']
			), 1, true);
