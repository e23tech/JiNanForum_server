	...
	if($chatpmmember) {
		...
		if($touid) {
			...
			if(!$daterange) {
				...
				$msg = lang('message', 'parameters_error');
...
if(!empty($list)) {
	...
	foreach ($list as $key => $value) {
		...
		/*MB add*/
		//$list[$key]['lastsummary'] = cutstr($pm['lastsummary'], 10);
    	//$lastsummary = str_replace('\"','', $value['lastsummary']);		
		//$value['lastsummary'] = preg_replace("~src=\"(?!http:)(\w+)~i", 'src="'.getsiteurl().'\\1', $lastsummary);		
		$value['datetime'] = strip_tags(dgmdate($value['dateline'], 'u'));
	
		if($value['message']){	
			
			require_once libfile('function/discuzcode');
			loadcache(array('smilies', 'smileytypes'));
			parsesmiles($value['message']);
			parsesmiles($value['lastsummary']);
			$value['message'] = preg_replace("~src=\"(?!http:)(\w+)~i", 'src="'.getsiteurl().'\\1', $value['message']);
			$value['lastsummary'] = preg_replace("~src=\"(?!http:)(\w+)~i", 'src="'.getsiteurl().'\\1', $value['lastsummary']);
		}
//		$value['tousername'] = cutstr($value['tousername'], 5);
		$value['tousername'] = $value['tousername'];
...
$title = ''; //标题栏文字
if(in_array($_G['gp_subop'], array('view'))) {
	$title = '<a href="javascript:history.back();">['.rpclang ( 'home', 'pm_back' ).']</a>';
	if($membernum >= 2 && $subject) {
		$title .= $membernum.rpclang ('home','pm_topic').':'.$subject;
	}else if($tousername) {
		$title .= rpclang('home','pm_about').$tousername.rpclang('home','pm_talk');
	} 
} else { 
	$title = rpclang('home','pm_title');
}

//分页
if($_GET['subop'] == "view"){
	$multi = multi($count, $perpage, $page, "pm.html?subop=view&touid=$touid");
	/*MB add start*/
    $jsonarr['zywy_curpage'] = $_G["zywy_curpage"];
	$jsonarr['zywy_totalpage'] = $_G["zywy_totalpage"];
 	/*MB add end*/
}else{
	$multi = multi($count, $perpage, $page, "pm.html?filter=$filter");
	/*MB add start*/
 	$jsonarr['zywy_curpage'] = $_G["zywy_curpage"];
	$jsonarr['zywy_totalpage'] = $_G["zywy_totalpage"];
 	/*MB add end*/
...
$jsonarr['tousername'] = $tousername; //接收方用户名
$jsonarr['list'] = $list; //消息数组
$jsonarr['title'] = $title; //标题栏文字
$jsonarr['page'] = $multi; //分页
