...
if($_GET['op'] == 'delete') {
	...
	if(!checkperm('manageshare')) {
		...
		$msg = lang('message', 'no_privilege_edithot_share');
	...
	if(!checkperm('allowshare')) {
		$msg = rpclang('home', 'no_privilege_share');
		jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
	}
	...
	switch ($type) {
		...
		case 'blog':
			...
			if(!$blog = DB::fetch($query)) {
				$msg = lang('message', 'blog_does_not_exist');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if(in_array($blog['status'], array(1, 2))) {
				$msg = lang('message', 'moderate_blog_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if($blog['friend']) {
				$msg = lang('message', 'logs_can_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
		...
		case 'album':
			...
			if(!$album = DB::fetch($query)) {
				$msg = lang('message', 'album_does_not_exist');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if($album['friend']) {
				$msg = lang('message', 'album_can_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
		...
		case 'pic':
			...
			if(!$pic = DB::fetch($query)) {
				$msg = lang('message', 'image_does_not_exist');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if(in_array($pic['status'], array(1, 2))) {
				$msg = lang('message', 'moderate_pic_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if($pic['friend']) {
				$msg = lang('message', 'image_can_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
		...
		case 'thread':
			...
			if(in_array($thread['displayorder'], array(-2, -3))) {
				$msg = lang('message', 'moderate_thread_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
		...
		case 'article':
			...
			if(!$article = DB::fetch($query)) {
				$msg = lang('message', 'article_does_not_exist');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			if(in_array($article['status'], array(1, 2))) {
				$msg = lang('message', 'moderate_article_not_share');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
	...
	if(submitcheck('sharesubmit', 0, $seccodecheck, $secqaacheck)) {
		...
		if($type != 'link') {
			if(DB::result_first("SELECT COUNT(*) FROM ".DB::table('home_share')." WHERE uid='$_G[uid]' AND itemid='$id' AND type='$type'")) {
				$msg = lang('message', 'spacecp_share_repeat');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
		}
		...
		if($type == 'link') {
			...
			if(empty($link)) {
				$msg = lang('message', 'url_incorrect_format');
				jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			}
			$arr['itemid'] = '0';
			$arr['fromuid'] = '0';
		...
		if($_G['gp_iscomment'] && $_POST['general'] && $commentcable[$type] && $id) {
			...
			if($currenttype == 'article') {
				...
				if(!$article['allowcomment']) {
					$msg = lang('message', 'no_privilege_commentadd');
					jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
			...
			if($currenttype == 'thread') {
				...
				if(!checkperm('allowcommentarticle')) {
					$msg = lang('message', 'group_nopermission', array('grouptitle' => $_G['group']['grouptitle']));
					jsonexit("{\"status\":\"0\",\"message\":\"$msg\"}");
				}
		...
		if(CHARSET != 'utf-8') {
			$_POST['general'] = utf8togbk($_POST['general']);
		}
		...
		$msg = lang('message', $showmessagecontent);
